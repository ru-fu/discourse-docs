...
Each worker node runs `LXD <https://linuxcontainers.org/>`_ in
...
6. You **MUST NOT** remove a LXD database node (check ``lxc cluster ls``
   ...
   `LXD <https://linuxcontainers.org/lxd/introduction/>`_ and its `raft implementation <https://github.com/canonical/raft>`_, this might
